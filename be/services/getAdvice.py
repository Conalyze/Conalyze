...
def format_for_frontend(gpt_result):
    ...
    for item in gpt_result.get("법령분석", []):
        law_list.append({
            ...
            "violation_reason": item.get("위반사항및법적해석", "").strip(),
            "detail_analysis": item.get("상세분석", "").strip()
        })

    formatted_result = {
        "law_analysis": law_list,
        "missing_essentials": gpt_result.get("필수사항누락", []),
        "wage_structure_review": gpt_result.get("임금구조평가", ""),
        "social_insurance_review": gpt_result.get("사회보험평가", ""),
        "additional_notes": gpt_result.get("기타유의사항", []),
        "overall_opinion": gpt_result.get("총평", "")
    }
